...
export default function SeriesCard(props) {
    ...
    return (<div className="bg-zinc-900 rounded-xl overflow-hidden hover:scale-105 transition">
      
      {id < 1_000_000_000 ? (<Link to={`/series/${id}`}>
          <img src={imgSrc} alt={title} className="h-72 w-full object-cover"/>
        </Link>) : (<Link to={`/series/${id}`}>
          <img src={imgSrc} alt={title} className="h-72 w-full object-cover"/>
        </Link>)}
      ...
      <div className="p-4">
        ...
        <button onClick={() => toggleFavourite(props)} className="mt-2 text-pink-400">
